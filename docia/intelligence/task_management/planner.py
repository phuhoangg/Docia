"""
Adaptive planner for Docia RAG Orchestrator
Creates and dynamically updates task plans based on orchestrator findings
"""
...
class Planner:
    """
    Adaptive planner that can create and modify task plans based on findings
    Key feature: Orchestrator can add/remove/modify tasks based on what it learns
    """
    ...
    def _parse_initial_plan(self, result: str, query: str, documents: Optional[List[Document]] = None) -> TaskPlan:
        ...
        try:
            ...
            for task_data in plan_data.get("tasks", []):
                ...
                # Parse information type with validation
                info_type = task_data.get("information_type", "basic")
                if info_type not in ["basic", "table", "chart", "image"]:
                    info_type = "basic"  # Default to basic if invalid
        ...
        try:
            ...
            if action == "continue":
                ...
                for task_data in new_tasks_data:
                    ...
                    logger.info(f"Added new task: {new_task.name} - Document: {assigned_doc} - Type: {info_type}")
